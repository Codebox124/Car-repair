...
const Whyus = () => {
  ...
  const vehicles = [
    ...
    { 
      id: 3, 
      image: "/DSC_4438.jpeg", 
      alt: "Silver cybertruck style vehicle" 
    },
    { 
      id: 4, 
      image: "/DSC_4443.jpeg", 
      alt: "Orange McLaren with doors open" 
    },
    { 
      id: 5, 
      image: "/DSC_4458.jpeg", 
  ...
  return (
    <div className="bg-black text-white py-16">
      <div className="container mx-auto px-4">
        ...
        >
          Why Are We The Best In Houston? See Below!
        </motion.h2>
        ...
        <div ref={containerRef} className="relative w-full overflow-hidden">
          ...
          <div className="flex justify-center mt-4 overflow-x-auto py-2">
            {Array.from({ length: maxIndex + 1 }).map((_, i) => (
              <button
                key={i}
                className={`w-3 h-3 mx-1 rounded-full flex-shrink-0 ${currentIndex === i ? 'bg-orange-500' : 'bg-gray-600'}`}
        ...
        <div className="flex justify-center mt-8">
          <motion.button
            className="bg-orange-500 text-white px-6 py-3 rounded-md font-bold flex items-center hover:bg-orange-600 transition-colors"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            <span className="mr-2">👁️</span> SEE MORE
          </motion.button>
        </div>
      </div>
    </div>
